...
for k in range (1,K+1):
    ...
    dsigma = NewtonIterFull (mesh1 ,w1 ,sigma0,alpha ,Hdiff1 ,V1FE,V1FEv) 
    ...
    #-------------- Checking for semi-convergene
    if err[k-1]<err[k]:
        if sw==0:
            print ("Semi-Convergence")
            sigmaTemp = project (sigmaTemp ,V1,solver_type='bicgstab',preconditioner_type='sor')
            File('Sigma_Case_A2_SC.pvd')<<sigmaTemp
            sw=1
    else:
        sigmaTemp=sigma0
...
sigma0 = project (sigma0 ,V1,solver_type='bicgstab',preconditioner_type='sor')
File('Sigma_Case_A2.pvd.pvd')<<sigma0